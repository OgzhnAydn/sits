...
const DURUM_RENK: Record<string, string> = { "aktif-tuzak": "var(--c-f5222d)", "canli": "var(--c-4d9fe0)", "park": "var(--c-8c8c8c)", "yayinda-degil": "var(--c-495a6e)", "belirsiz": "var(--c-5b6b7d)" };
...
function Baslik({ ikon, children }: { ikon: React.ReactNode; children: React.ReactNode }) {
  return <Flex align="center" gap={8} style={{ marginBottom: 14 }}><span style={{ color: "var(--c-4d9fe0)" }}>{ikon}</span><Text strong style={{ fontSize: 13, letterSpacing: ".04em", textTransform: "uppercase", color: "var(--c-c7d6e6)" }}>{children}</Text></Flex>;
}
      ...
      <Flex align="flex-end" gap={2} style={{ height: 130 }}>
        {veri.map((v, i) => (
          <div key={i} title={`${v.gun}: ${v.sayi}`} style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "flex-end", height: "100%" }}>
            <div style={{ height: `${(v.sayi / max) * 100}%`, minHeight: v.sayi ? 3 : 0, background: "linear-gradient(180deg,var(--c-4d9fe0),var(--c-2a6aa8))", borderRadius: "3px 3px 0 0" }} />
          </div>
        ))}
      </Flex>
      <Flex justify="space-between" style={{ marginTop: 6 }}>
        <Text style={{ fontSize: 10, color: "var(--c-5b6b7d)" }}>{veri[0]?.gun}</Text>
        <Text style={{ fontSize: 10, color: "var(--c-5b6b7d)" }}>{veri[Math.floor(veri.length / 2)]?.gun}</Text>
        <Text style={{ fontSize: 10, color: "var(--c-5b6b7d)" }}>{veri[veri.length - 1]?.gun}</Text>
      </Flex>
    </div>
  );
}
  ...
  return (
    <Flex align="center" gap={20} wrap>
      <svg width={130} height={130} viewBox="0 0 130 130">
        <circle cx={65} cy={65} r={r} fill="none" stroke="var(--c-17293c)" strokeWidth={16} />
        {parcalar.map(([k, v], i) => { const uz = (v / toplam) * c; const el = <circle key={i} cx={65} cy={65} r={r} fill="none" stroke={DURUM_RENK[k] || "var(--c-5b6b7d)"} strokeWidth={16} strokeDasharray={`${uz} ${c - uz}`} strokeDashoffset={-ofset} transform="rotate(-90 65 65)" />; ofset += uz; return el; })}
        <text x={65} y={61} textAnchor="middle" fill="var(--c-e6eef7)" fontSize={22} fontWeight={700}>{fmt(toplam)}</text>
        <text x={65} y={78} textAnchor="middle" fill="var(--c-5b6b7d)" fontSize={10}>toplam</text>
      </svg>
      <Flex vertical gap={6} style={{ flex: 1, minWidth: 130 }}>
        {parcalar.map(([k, v]) => (
          <Flex key={k} align="center" gap={8}>
            <span style={{ width: 10, height: 10, borderRadius: 3, background: DURUM_RENK[k] || "var(--c-5b6b7d)" }} />
            <Text style={{ fontSize: 12, color: "var(--c-c7d6e6)", flex: 1 }}>{DURUM_AD[k] || k}</Text>
            <Text strong style={{ fontSize: 12, color: "var(--c-e6eef7)" }}>{v}</Text>
          </Flex>
        ))}
      </Flex>
    </Flex>
...
function YatayBar({ veri, renk = "var(--c-4d9fe0)", bos = "Henüz veri yok" }: { veri: Say[]; renk?: string; bos?: string }) {
  if (!veri.length) return <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<Text style={{ color: "var(--c-5b6b7d)", fontSize: 12 }}>{bos}</Text>} />;
  const max = Math.max(1, ...veri.map((v) => v.sayi));
  return (
    <Flex vertical gap={9}>
      {veri.map((v, i) => (
        <div key={i}>
          <Flex justify="space-between" style={{ marginBottom: 3 }}>
            <Text style={{ fontSize: 12, color: "var(--c-c7d6e6)", maxWidth: "78%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={v.ad}>{v.ad}</Text>
            <Text strong style={{ fontSize: 12, color: "var(--c-e6eef7)" }}>{v.sayi}</Text>
          </Flex>
          <div style={{ height: 6, background: "var(--c-17293c)", borderRadius: 4 }}><div style={{ width: `${(v.sayi / max) * 100}%`, height: "100%", background: renk, borderRadius: 4 }} /></div>
        </div>
      ))}
    </Flex>
  );
      ...
      <Flex gap={3} wrap>
        {veri.map((v, s) => (
          <Flex key={s} vertical align="center" gap={3} style={{ flex: "1 0 3.2%" }}>
            <div title={`${s}:00 — ${v} tespit`} style={{ width: "100%", height: 26, borderRadius: 4, background: v ? `rgba(77,159,224,${0.15 + 0.85 * (v / max)})` : "var(--c-17293c)" }} />
            {s % 3 === 0 && <Text style={{ fontSize: 9, color: "var(--c-5b6b7d)" }}>{s}</Text>}
          </Flex>
        ))}
      </Flex>
      <Text style={{ fontSize: 11, color: "var(--c-5b6b7d)" }}>Saat bazında yeni tespit yoğunluğu (UTC)</Text>
    </div>
  );
}
...
export default function AnalitikPanel({ marka }: { marka: string }) {
  ...
  if (!marka) return <Card style={KART}><Empty description={<Text style={{ color: "var(--c-8fa6bd)" }}>Analitik panel marka-kilitli bir hesap gerektirir.</Text>} /></Card>;
    ...
    <Flex vertical gap={12}>
      ...
      <Row gutter={[12, 12]}>
        {[
          ...
          { t: "Bugün", v: veri.saglik.buGun, i: <ThunderboltOutlined />, c: "var(--c-52c41a)" },
          { t: "Bu Hafta", v: veri.haftalik, i: <RiseOutlined />, c: "var(--c-4d9fe0)" },
          { t: "Bu Ay", v: veri.aylik, i: <DashboardOutlined />, c: "var(--c-4d9fe0)" },
          { t: "Aktif Tuzak", v: veri.durum["aktif-tuzak"] || 0, i: <ThunderboltOutlined />, c: "var(--c-f5222d)" },
          { t: "Canlı", v: veri.durum["canli"] || 0, i: <GlobalOutlined />, c: "var(--c-4d9fe0)" },
          { t: "USOM'da Yok", v: veri.erkenlik.usomdaYok, i: <SafetyCertificateOutlined />, c: "var(--c-f2a33c)" },
        ].map((k) => (
          ...
          <Card style={KART} styles={{ body: { padding: 18 } }}>
            ...
            <Flex align="flex-end" gap={10} style={{ height: 130 }}>
              {veri.riskHist.map((r) => {
                ...
                const renk = r.aralik === "80-100" || r.aralik === "60-80" ? "var(--c-f5222d)" : r.aralik === "40-60" ? "var(--c-fa8c16)" : "var(--c-5b6b7d)";
                return (
                  <Flex key={r.aralik} vertical align="center" justify="flex-end" style={{ flex: 1, height: "100%" }}>
                    <Text style={{ fontSize: 11, color: "var(--c-e6eef7)" }}>{r.sayi}</Text>
                    <div style={{ width: "100%", height: `${(r.sayi / max) * 88}%`, minHeight: r.sayi ? 3 : 0, background: renk, borderRadius: "4px 4px 0 0" }} />
                    <Text style={{ fontSize: 10, color: "var(--c-5b6b7d)", marginTop: 4 }}>{r.aralik}</Text>
                  </Flex>
                );
              })}
            </Flex>
      ...
      <Row gutter={[12, 12]}>
        ...
        <Col xs={24} sm={12} lg={8}><Card style={KART} styles={{ body: { padding: 18 } }}><Text strong style={{ color: "var(--c-8fa6bd)", fontSize: 12 }}>Ortak ağ (ASN)</Text><div style={{ height: 10 }} /><YatayBar veri={veri.ortak.asn} renk="var(--c-fa8c16)" /></Card></Col>
        <Col xs={24} sm={12} lg={8}><Card style={KART} styles={{ body: { padding: 18 } }}><Text strong style={{ color: "var(--c-8fa6bd)", fontSize: 12 }}><GlobalOutlined /> Barındırma ülkesi</Text><div style={{ height: 10 }} /><YatayBar veri={veri.ortak.ulke} renk="var(--c-4d9fe0)" /></Card></Col>
        <Col xs={24} sm={12} lg={8}><Card style={KART} styles={{ body: { padding: 18 } }}><Text strong style={{ color: "var(--c-8fa6bd)", fontSize: 12 }}>Sertifika sağlayıcı (CA)</Text><div style={{ height: 10 }} /><YatayBar veri={veri.ortak.ca} renk="var(--c-52c41a)" /></Card></Col>
        <Col xs={24} sm={12} lg={8}><Card style={KART} styles={{ body: { padding: 18 } }}><Text strong style={{ color: "var(--c-8fa6bd)", fontSize: 12 }}>En çok kötüye kullanılan uzantı</Text><div style={{ height: 10 }} /><YatayBar veri={veri.tld} renk="var(--c-7c5cff)" /></Card></Col>
      ...
      {app && (
        <Card style={KART} styles={{ body: { padding: 18 } }}>
          ...
          {app.toplam === 0 ? (
            <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<Text style={{ color: "var(--c-5b6b7d)", fontSize: 12 }}>App Store’da bu markayı taşıyan uygulama bulunmadı.</Text>} />
          ) : (
            <Flex vertical gap={8}>
              {app.sonuc.slice(0, 10).map((a) => (
                <a key={a.paket} href={a.url} target="_blank" rel="noreferrer"
                  className="app-satir" style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 12px", border: "1px solid var(--c-17293c)", borderRadius: 10, borderLeft: `3px solid ${a.resmiMi ? "var(--c-3ee08a)" : "var(--c-f5222d)"}`, textDecoration: "none" }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  {a.ikon ? <img src={a.ikon} alt="" style={{ width: 34, height: 34, borderRadius: 8 }} /> : <span style={{ width: 34, height: 34, borderRadius: 8, background: "var(--c-12283f)" }} />}
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <Text style={{ color: "var(--c-e6eef7)", fontSize: 13, display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.ad}</Text>
                    <Text style={{ color: "var(--c-8fb0d4)", fontSize: 11 }}><span style={{ color: a.platform === "ios" ? "var(--c-c7d6e6)" : "var(--c-3ee08a)", fontWeight: 600 }}>{a.platform === "ios" ? "iOS" : "Android"}</span> · {a.gelistirici}</Text>
                  </div>
                  {a.puan ? <Text style={{ color: "var(--c-f2a33c)", fontSize: 12, fontWeight: 600 }}>★ {a.puan.toFixed(1)}</Text> : null}
                  {a.resmiMi
          ...
          <Text style={{ display: "block", marginTop: 10, fontSize: 11, color: "var(--c-5b6b7d)" }}>{app.not} “İncele”, marka adını taşıyan ama resmî geliştiriciden olmayan uygulamadır — taklit olabilir, doğrulanmalı.</Text>
        </Card>
      )}
      ...
      {reklam && (
        <Card style={KART} styles={{ body: { padding: 18 } }}>
          ...
          {!reklam.yapilandirildi ? (
            <Text style={{ fontSize: 12, color: "var(--c-8fb0d4)" }}>{reklam.not}</Text>
          ) : reklam.reklamlar.length === 0 ? (
            <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<Text style={{ color: "var(--c-5b6b7d)", fontSize: 12 }}>{reklam.not}</Text>} />
          ) : (
            <Flex vertical gap={8}>
              {reklam.reklamlar.slice(0, 10).map((r, i) => (
                <a key={i} href={r.snapshot || "#"} target="_blank" rel="noreferrer"
                  style={{ display: "block", padding: "9px 12px", border: "1px solid var(--c-17293c)", borderRadius: 10, borderLeft: `3px solid ${r.supheli ? "var(--c-f5222d)" : "var(--c-3ee08a)"}`, textDecoration: "none" }}>
                  ...
                  {r.baslik && <Text style={{ display: "block", color: "var(--c-c7d6e6)", fontSize: 12, marginTop: 2 }}>{r.baslik}</Text>}
                  <Flex align="center" gap={8} style={{ marginTop: 3 }}>
                    {r.hedefAlan && <Text style={{ color: r.supheli ? "var(--c-ff9aa4)" : "var(--c-8fb0d4)", fontSize: 11, fontFamily: "monospace" }}>→ {r.hedefAlan}</Text>}
                    {r.platformlar?.length ? <Text style={{ color: "var(--c-5b6b7d)", fontSize: 10 }}>{r.platformlar.join(", ")}</Text> : null}
                  </Flex>
                </a>
              ))}
            </Flex>
        ...
        <Col xs={24} lg={16}>
          <Card style={KART} styles={{ body: { padding: 18 } }}>
            ...
            {veri.yukselmeler.length ? (
              ...
              <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<Text style={{ color: "var(--c-5b6b7d)", fontSize: 12 }}>Henüz eyleme geçen aday yok — sistem izlemeye devam ediyor. Bir aday park→canlı geçtiğinde burada belirir.</Text>} />
            )}
          </Card>
        </Col>
        <Col xs={24} lg={8}>
          <Card style={KART} styles={{ body: { padding: 18 } }}>
            ...
            <Statistic value={veri.erkenlik.usomdaYok} suffix={`/ ${veri.erkenlik.toplam}`} valueStyle={{ color: "var(--c-f2a33c)", fontWeight: 700 }} />
            <Text style={{ fontSize: 12, color: "var(--c-8fa6bd)" }}>tespit ulusal listede (USOM) HİÇ yok — resmî radardan önce yakaladık.</Text>
            <div style={{ height: 14 }} />
            <Progress percent={veri.saglik.intelKapsam} strokeColor="var(--c-4d9fe0)" trailColor="var(--c-17293c)" format={(p) => <Text style={{ color: "var(--c-8fa6bd)", fontSize: 12 }}>%{p} veri kapsamı · son tespit {gecenSure(veri.saglik.sonTespit)}</Text>} />
          </Card>
        </Col>
      </Row>
    </Flex>
